...
var tick = (elapsedTime, multiplier) => {
    ...
    let r9bonus = 125000 // R9 bonus value for 1000 students

    if (bonus == 1) {
        r9bonus = 1;
    }

    q1 = q1 + r9bonus * dt * getDQ1(dq1.level) * q2;
    q2 = q2 + r9bonus * dt * getDQ2(dq2.level) * (qTerms.level > 0 ? q3 : BigNumber.ONE);
    if (qTerms.level > 0) q3 = q3 + r9bonus * dt * getDQ3(dq3.level) * (qTerms.level > 1 ? q4 : BigNumber.ONE);
    if (qTerms.level > 1) q4 = q4 + r9bonus * dt * getDQ4(dq4.level);

    r1 = r1 + r9bonus * dt * getDR1(dr1.level) * r2;
    r2 = r2 + r9bonus * dt * getDR2(dr2.level) * (rTerms.level > 0 ? r3 : BigNumber.ONE);
    if (rTerms.level > 0) r3 = r3 + r9bonus * dt * getDR3(dr3.level) * (rTerms.level > 1 ? r4 : BigNumber.ONE);
    if (rTerms.level > 1) r4 = r4 + r9bonus * dt * getDR4(dr4.level);

    currency.value += bonus * r9bonus * dt * (q1.pow(getQ1Exp(q1Exp.level)) *
